...
check_home_owner() {
  ...
  if [ "$owner" != "$user_id" ]; then
      ...
      for excep in $EXCEPTIONS; do
          if [ "$home:$user_id:$owner" = "$excep" ]; then
              ok "The home directory ($home) of user $user_id is owned by $owner but is part of exceptions ($home:$user_id:$owner)."
              EXCEP_FOUND=1
              break
          fi
      done
      if [ "$EXCEP_FOUND" -eq 0 ]; then
          crit "The home directory ($home) of user $user_id is owned by $owner."
          FNRET=1
      fi
  fi
}
...
check_home_perm() {
    ...
    if echo "$HOME_PERM_EXCEPTIONS" | grep -q "$dir"; then
