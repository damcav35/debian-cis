...
audit() {
    ...
    if [ -n "$IGNORED_PATH" ]; then
        ...
        [ "${FIND_IGNORE_NOSUCHFILE_ERR}" = true ] && set +e
        # shellcheck disable=2086
        FOUND_SUID_BINARIES=$($SUDO_CMD find $FS_NAMES -xdev -ignore_readdir_race -type f -perm -4000 -print)
        # shellcheck disable=2086
        FOUND_SGID_BINARIES=$($SUDO_CMD find $FS_NAMES -xdev -ignore_readdir_race -type f -perm -2000 -print)
        [ "${FIND_IGNORE_NOSUCHFILE_ERR}" = true ] && set -e
    fi

    BAD_SUID_BINARIES=""
    for BINARY in $FOUND_SUID_BINARIES; do
        if grep -qw "$BINARY" <<<"$EXCEPTIONS"; then
            debug "$BINARY is confirmed as an exception"
        else
            BAD_SUID_BINARIES="$BAD_SUID_BINARIES $BINARY"
        fi
    done

    BAD_SGID_BINARIES=""
    for BINARY in $FOUND_SGID_BINARIES; do
        if grep -qw "$BINARY" <<<"$EXCEPTIONS"; then
            debug "$BINARY is confirmed as an exception"
        else
            BAD_SGID_BINARIES="$BAD_SGID_BINARIES $BINARY"
        fi
    done

    if [ -n "$BAD_SUID_BINARIES" ]; then
        crit "Some suid files are present"
        # shellcheck disable=SC2001
        FORMATTED_RESULT=$(sed "s/ /\n/g" <<<"$BAD_SUID_BINARIES" | sort | uniq | tr '\n' ' ')
        crit "$FORMATTED_RESULT"
    else
        ok "No unknown suid files found"
    fi

    if [ -n "$BAD_SGID_BINARIES" ]; then
        crit "Some sgid files are present"
        # shellcheck disable=SC2001
        FORMATTED_RESULT=$(sed "s/ /\n/g" <<<"$BAD_SGID_BINARIES" | sort | uniq | tr '\n' ' ')
        crit "$FORMATTED_RESULT"
    else
...
apply() {
    info "Removing suid or sgid on valid binary may seriously harm your system, report only here"
}
